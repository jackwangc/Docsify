...
2. 读已提交（Read Committed） 大多数数据库默认的隔离级别
   ...
   3. 缺点：不可重复读，在同一个事务中如果两次读取相同的数据时，最后的结果却不一致。
...
3. 可重复读（Repeatable-Read) mysql 数据库所默认的级别
   ...
   3. 幻读：当用户读取某一范围的数据行时，另一个事务又在该范围内插入了新行，当用户再读取该范围的数据行时，会发现有新的“幻影” 行，插入数据时发生
   4. innodb 采用 mvcc 部分解决幻读问题
4. 序列化（serializable）
...
1. mysqldumpslow
   1. 慢查询日志
   2. `mysqldumpslow -s r -t 20 host-slow.log`, 看出访问次数最多的20个sql语句和返回记录集最多的20个sql
2. mysql profile
   1. 通过该工具可以获取一条Query 在整个执行过程中多种资源的消耗情况，如 CPU，IO，IPC，SWAP 等 
   2. 使用 `show profile` , 查询最近一条语句的信息
   3. 使用 `show profiles`, 查询执行语句列表
   4. `show profile block io,cpu for query 6`, 查看指定语句的 cpu block io 消耗
3. mysql explain
   1. 使用 `EXPLAIN SELECT * from user_info WHERE id < 300;` 
